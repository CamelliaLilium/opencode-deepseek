...
Superpowers 是 OpenCode 内置的核心工作流技能，影响 Agent 的行为模式。高频有用的几个：

- **using-superpowers** — 告诉 Agent 怎么自动发现和加载匹配的 Skill（内置，无需安装）
- **brainstorming** — 任何创造性任务之前先做需求澄清和方案设计（内置，无需安装）
- **systematic-debugging** — 遇到 bug 先系统性分析，而不是盲目试错（内置，无需安装）
- **test-driven-development** — TDD 红→绿→重构流程（内置，无需安装）

这些是 OpenCode 自带的，只要在 prompt 里提对应的关键词就会自动触发。

### 3. SCI-Writing 系列（学术科研党必备）

我的主力科研工具链，从读论文到写论文到做 PPT 一条龙：
...
- **web-access** — 所有联网操作统一通道（搜索、网页抓取、社交媒体读取）
- **parallel-web** — 学术文献优先的并行搜索工具

### 其他推荐

- **architecture-diagram** — 画架构图（Mermaid / PlantUML / D2），写文档和报告时高频使用
- **git-hygiene** — 自动清理 git 工作区，防止 diff 爆炸
...
OpenCode 是开源的，自己配置模型 API 就能免费使用。Claude Code 是 Anthropic 的商业产品。体验上，OpenCode Agent 的工作方式更像 Claude Code（对话式编程），不像 Cursor 那样能在编辑器里直接改代码。

**Q：需要什么硬件配置？**

不需要本地 GPU。OpenCode 通过 API 调用模型，任何能跑 Node.js 的电脑都可以。

**Q：API Key 安全吗？**

API Key 存在本地，不上传到代码仓库。OpenCode 本身也不存储你的代码数据。

**Q：Skills 装太多会不会拖慢速度？**

每个 Skill 的 `SKILL.md` 会被加载到上下文中，确实会消耗一些 token。建议按我上面的分类，先装核心的几类，后面按需添加。
...
| Karpathy 编码原则 | https://github.com/multica-ai/andrej-karpathy-skills |
| 社区 Skills 合集 | https://github.com/sickn33/antigravity-awesome-skills |
